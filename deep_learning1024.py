...
#    array_file=np.array(all_list)
    ...
    label=[]
    # ran_list=[x for x in range(17369)]
...
#    plt.legend()
    #print(all_data)
    vali_data=all_data[3001:3985]
    #print(vali_data)
    test_label=label[:3000]
    #print(test_label)
    model=MLP(n_hidden=35,n_deep=4, l1_norm=0, l2_norm=0, drop=0.4, early_stop=True,verbose=1, learning_rate=0.1,max_epoch=245,activation='relu',optimizer='sgd')
    #MLP()
    #print('1')
    ...
    print(non_label[3001:3985])
    c=0
    for i in range(len(pre_label)):
        if pre_label[i]==non_label[3001:3985][i]:
            c+=1
    print(c)
